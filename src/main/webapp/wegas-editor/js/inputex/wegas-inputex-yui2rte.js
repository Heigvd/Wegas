...
YUI.add("wegas-inputex-rte", function (Y) {
	 ...
	 */
        renderComponent: function () {
            ...
            var cfg = {                                                         //This is the default config
                width: '100%',
                //  height: '300px',
                autoHeight: true,
                dompath: false,
            ...
            var i, o = this.options.opts;                                       //The options object
            for (i in o) {                                                      //Walk it to set the new config object
            ...
            if (editorType) {                                                   //If this fails then the code is not loaded on the page
                this.editor = new editorType(id, cfg);
            } else {
            ...
            this.editor.filter_msword = function (html) {
                ...
                if (!this.get('filterWord')) {                                  // if we don't filter ms word junk
                    return html;
                }
            ...
            this.editor.addListener('toolbarLoaded', function (e, o) {
                this.editor.subscribe('afterOpenWindow', function (e) {         // afterOpenWindow or windowRender
                    var value = null,
                    targetNode = new Y.Node(e.win.body);
                    switch (e.win.name) {
                        case 'insertimage':
