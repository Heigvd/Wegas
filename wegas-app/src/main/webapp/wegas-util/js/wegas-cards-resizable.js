    ...
    Y.Wegas.CardsResizable = Y.Base.create("wegas-cards-resizable", Y.Plugin.Base, [Y.Wegas.Plugin, Y.Wegas.Editable], {
        ...
        _initValues: function() {
            ...
            card.blocs.forEach(function(bloc) {
                context.size.blocs[bloc.type].large += 2;
                context.size.blocs[bloc.type].medium += 2;
                context.size.blocs[bloc.type].small += (bloc.type === "action" ? 2 : 0);
                bloc.items.forEach(function() {
                    context.size.blocs[bloc.type].cases = context.size.blocs[bloc.type].cases + 1;
                    context.size.blocs[bloc.type].large += ((bloc.type === "monitoring") ? 80 : 65);
                    context.size.blocs[bloc.type].medium += 65;
                    context.size.blocs[bloc.type].small += 65;
                });
            });
