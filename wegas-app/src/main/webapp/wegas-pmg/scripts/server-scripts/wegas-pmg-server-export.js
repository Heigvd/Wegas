...
var PMGExport = (function() {
    ...
    function findQuestionAndResult(message){
        var  qDesc, resultAnswer, type, i, j, k, l,
        ...
        for (i=0 ; i<candidates.size(); i++){ // Question Iterator
            question = candidates.get(i);
            for (j=0; j<question.getItems().size(); j++){ // Choice iterator
                choice = question.getItems().get(j);
                for (k=0; k<choice.getResults().size(); k++){
                    result = choice.getResults().get(k);
                    if (result.getAnswer().match(textAnswer)){
                        // MATCH FOUND
                        return {"question" : question.getName(), "result" : k};
                    }
                }
            }
        }
