...
YUI.add("wegas-pmg-datatable", function(Y) {
    ...
    var CONTENTBOX = "contentBox", Datatable, micro = new Y.Template(),
        ...
        assignedRessource: micro.compile('<% var gras=false, currentPeriode = Y.Wegas.Facade.VariableDescriptor.cache.find("name", "periodPhase2").getInstance().get("value"); for(var i = 0; i < this.length; i+=1){ var occupations = this[i].ressourceInstance.get("occupations"); for(var oi = 0; oi<occupations.length; oi++) { if(occupations[oi].get("time")==currentPeriode && occupations[oi].get("editable")){gras=true; break;}} for (var j in this[i].ressourceInstance.get("skillsets")){ if (gras) {%> <p style="font-weight: bold;"><%} else { %> <p><% } %><%= this[i].ressourceDescriptor.get("label") %> (<%= j %> <%= Y.Wegas.PmgDatatable.TEXTUAL_SKILL_LEVEL[this[i].ressourceInstance.get("skillsets")[j]]%>)</p><% }} %>')
    };
