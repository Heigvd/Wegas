...
import { IDialogueState, IState, ITransition } from "wegas-ts-api";
import { FlowLine, Process } from "./FlowChart";
import { ProcessProps, CustomProcessComponent } from "./ProcessComponent";
import { StateProcess } from "../../Editor/Components/StateMachineEditor";
import { entityIs } from "../../data/entities";
import { translate } from "../../Editor/Components/FormView/translatable";
import { languagesCTX } from "../Contexts/LanguagesProvider";
import { IDialogueTransition } from "wegas-ts-api/typings/WegasEntities";
import { DnDFlowchartHandle, ProcessHandleProps, PROCESS_HANDLE_DND_TYPE } from "./Handles";
...
interface StateBoxProps {
  state: StateProcess<ITransition | IDialogueTransition, IState | IDialogueState>;
  className?: string;
  onClick?:(e: ModifierKeysEvent, process: StateProcess<ITransition | IDialogueTransition, IState | IDialogueState> ) => void;
  selected?: boolean;
}

export function StateBox({ state, className, onClick, selected }: StateBoxProps) {
  ...
  return (
    ...
    onClick = {(e)=>onClick && onClick(e, state)}>
      ...
      >
        ...
        <div>
          <p className="StateLabelTextStyle">{(entityIs(state.state, "State")? state.state.label : translate(state.state.text, lang))|| "Empty"}</p>
        </div>
        < StateProcessHandle
            sourceProcess = {state} />
      </div>
