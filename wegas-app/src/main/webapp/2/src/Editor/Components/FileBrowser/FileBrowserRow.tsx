...
const dndHover = css({
  ...
  const DndFileRow: React.FC<DndTargetFileRowProps> = (props: DndTargetFileRowProps) => {
    const isActive : boolean = props.canDrop && props.isOver;
    return props.connectDropTarget(
      <tr onClick={() => {props.onClick(props.file)}} className={isActive ? dndHover : undefined}>
        <FileRow
        file={props.file}
  ...
  export const DropTargetFileRow = DropTarget(
    ...
    export function AddFileRow(props : AddFileRowProps){
        return (<td onClick={props.onClick} className={uploadCell} colSpan={Number.MAX_SAFE_INTEGER}> Drag file or click there to upload </td>);  
    }
    
    const DndAddFileRow: React.FC<DndTargetAddFileRowProps> = (props: DndTargetAddFileRowProps) => {
        const isActive : boolean = props.canDrop && props.isOver;
        return props.connectDropTarget(
