...
function genChoices(items: (IVariableDescriptor | undefined)[],
    level: number,
    maxLevel: number,
    classFilter: string[],
    selectable?: number[]): Choices {
    ...
    if (items && level <= maxLevel) {
        items.forEach(i => {
            if (i) {
                if (entityIs<IListDescriptor>(i, 'ListDescriptor')) {
...
function FlatVariableSelect(
    props: IFlatVariableSelectProps
) {
    const {maxLevel, root, classFilter, selectableLevels, ...restView} = props.view;

    const filter: string[] =
        classFilter ? Array.isArray(classFilter) ? classFilter : [classFilter] : [];
    let items: any[];
    if (root) {
        if (!Array.isArray(root)) {
            ...
            items = root.map(name =>
                [VariableDescriptor.first('name', name)]
            );
        }
    } else {
    ...
    const choices = genChoices(items, 0, maxLevel || Infinity, filter, selectableLevels);
