        ...
        [Y.WidgetChild, Y.Wegas.Widget, Y.Wegas.Editable],
        {
            ...
            initializer: function() {
                ...
                if (this.get('xLabelMapper')) {
                    this._xLabelMapper = eval(
                        '(' + this.get('xLabelMapper') + ')'
                    );
                }
            },
            bindUI: function() {
            ...
            renderUI: function() {
                ...
                for (i = 0; i < variables.length; i += 1) {
                    vd = Y.Wegas.Facade.Variable.cache.find(
                        'name',
                        variables[i].name
                    );
                    ...
                    legendNode.append(
                        '<div><span class="color ct-series-' +
            ...
            syncUI: function() {
                ...
                for (i = 0; i < variables.length; i += 1) {
                    ...
                    if (!vd) {
                        this.showMessage(
                            'error',
                            'Variable ' + variables[i].name + ' not found'
                        );
                        return;
                    }
            ...
            updateHistory: function(vd, i) {
                var ctx = this,
                    promise = Y.Promise(function(resolve, reject) {
                        Y.Wegas.Facade.Variable.cache.getWithView(
                            vd.getInstance(),
                            'Extended',
                            {
                                on: {
                                    success: Y.bind(function(e) {
                                        ...
                                        resolve({
                                            serie: entity
                                                .get('history')
                                                .concat(entity.get('value')),
                                            label: label
                                        });
                                    }, ctx),
                                    failure: function(r) {
                                        Y.error(
                                            'Error by loading history data'
                                        );
                                        resolve({
                                            serie: [],
                                            label: vd.get('label')
