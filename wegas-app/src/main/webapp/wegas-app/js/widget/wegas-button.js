...
YUI.add('wegas-button', function(Y) {
    ...
    Y.Button.prototype._setLabel = function(label, name, opts) {
        if (!opts || opts.src !== 'internal') {
            this.set('labelHTML', Y.Template.Micro.compile(label || '')(), {
                src: 'internal'
            });
        }
        return label;
    ...
    MarkAsUnread = Y.Base.create(
        'wegas-mark-as-unread',
        Y.Plugin.UnreadCount,
        [],
        {
            setCounterValue: function(unreadCount) {
                if (unreadCount > 0) {
    ...
    Wegas.ToggleButton = Y.Base.create(
        'button',
        Y.ToggleButton,
        [Y.WidgetChild, Wegas.Widget, Wegas.Editable],
        {},
