...
YUI.add('wegas-file-uploader', function(Y) {
    ...
    var FileUploader = Y.Base.create('wegas-file-uploader', Y.Widget,
        ...
        CONTENT_TEMPLATE: ""
            + "<div>"
            + "   <div class='wegas-file-uploader--uploader'>"
            + "    <label>"
            + "      <span class='wegas-file-uploader--input-label'></span>"
            + "      <input class='wegas-file-uploader--input-input' style='display: none' type='file' />"
            + "    </label>"
            + "  </div>"
            + "  <div class='wegas-file-uploader--filename'>"
            + "  </div>"
            + "</div>"
        ,
        ...
        bindUI: function() {
            ...
            this.handlers.onRootDrop = this.get("root").get("boundingBox").on("drop", function(e) {
                e.preventDefault();
                e.halt(true);
                // prevent drop to avoid application exit
            }, this);
            CB.delegate("drop", function(e) {
                e.preventDefault();
                e.halt(true);
            }, ".wegas-file-uploader--uploader", this);
            CB.delegate("dragenter", function(e) {
                e.target.addClass("highlight");
            }, ".wegas-file-uploader--uploader", this);
            CB.delegate("dragleave", function(e) {
        ...
        getUploadLabel: function() {
            return this.get("uploadLabel");
        },
        getUpdateLabel: function() {
            return this.get("updateLabel");
        },
        syncUI: function() {
            ...
            if (data && data.filename) {
                theLabel.getDOMNode().innerText = this.getUpdateLabel();
            } else {
                theLabel.getDOMNode().innerText = this.getUploadLabel();
            }
         ...
         */
        readLink: function() {
            var theVar = this.get("shortcutVar.evaluated");
            var ret = {
                variable: theVar,
                displayName: '',
                filename: '',
