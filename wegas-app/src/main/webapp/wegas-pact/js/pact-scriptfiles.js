 ...
 */
YUI.add('pact-scriptfiles', function(Y) {
    'use strict';
     ...
     *  @constructor
     */
    ScriptFiles = Y.Base.create(
        'pact-scriptfiles',
        Y.Widget,
        [Y.WidgetChild, Wegas.Widget, Wegas.Editable],
        {
            CONTENT_TEMPLATE:
                "<div><div class='buttons'></div><div class='fileName'></div></div>",
            ...
            bindUI: function() {
                this.handlers.sync = Wegas.Facade.Variable.after(
                    'update',
                    this.syncUI,
                    this
                );
            ...
            syncUI: function() {
                Wegas.Facade.Variable.cache.getWithView(
                    this.get('variable.evaluated').getInstance(),
                    'Extended',
                    {
                        on: {
                            success: Y.bind(function(e) {
                                ...
                                var node,
                                    i,
                                    messages = e.response.entity.get(
                                        'messages'
                                    ),
                                    fileNameDiv = this.get(CONTENTBOX).one(
                                        '.fileName'
                                    );
            ...
            addFile: function(fileName) {
                Wegas.Facade.Variable.script.run(
                    "Variable.find(gameModel, 'files').sendMessage(self, '', '" +
                        fileName +
                        ".js', '', []);",
                    {
                        on: {
