...
YUI.add("pact-level", function (Y) {
  ...
  ProgGameLevel = Y.Base.create(
    ...
    [Y.WidgetChild, Wegas.Widget, Wegas.Editable],
    {
      ...
      _seqBuffer: "",
      _wait: null,
      _sequenceObject: Y.Wegas.Facade.Variable.cache.find(
        "name",
        SEQUENCE_OBJECT
      ),
      addToSequence: function (event) {
        ...
        var now = new Date().getTime(),
          currentLevel = Y.Wegas.Facade.Variable.cache
            .find("name", CURRENT_LEVEL)
            .getInstance()
            .get("value");
        event.time = now;
        if (this._wait) {
      ...
      bindUI: function () {
        ...
        this.handlers.push(
          Y.one("body").on("key", this.onRunClick, "enter+shift", this)
        ); // Shift + enter event
      ...
      onRunClick: function (e) {
        ...
        if (this.get(STATE) === "run" || this.get(STATE) === "debugrun") {
          ...
          if (
            Y.Object.keys(
              this.mainEditorTab.aceField.getSession().getBreakpoints()
            ).length ||
      ...
      doPersist: function () {
        ...
        batchRemoteCall(
          function (HISTORY_INBOX, msgDate, msgSubject, msgBody) {
            Variable.find(gameModel, HISTORY_INBOX).sendDatedMessage(
              self,
              "",
              msgDate,
              msgSubject,
      ...
      resetUI: function () {
        ...
        this.get(CONTENTBOX).one(".play").show();
      },
      ...
      onReply: function (result, code) {
        Y.log("onReply(" + Y.JSON.stringify(result) + ")", INFO, PROGGAMELEVEL);
        ...
        switch (resultStatus) {
          case "success":
            ...
            batchRemoteCall(
              function (code, level, success) {
                var stmts = [Log.level(code, level, true, success)];
                if (success) {
                  stmts.push(Log.statement("completed", "level", level));
                }
                Log.post(stmts);
                if (success) {
          ...
          case "runtimeError":
            ...
            break;
          case "timeout":
            // This is not an exception generated by user code,
            // but by the specific way we detect infinite loops !
            // (distinction required by the abusive assumption that
            // every exception signals a syntax error)
            this.isRuntimeException = false;
            this.isSuccessful = false;
            this.feedback = result[1];
            break;
        ...
        Y.Wegas.Alerts.showNotification("Your script contains an error.", {
          timeout: 1e3,
        });
      },
      ...
      doIdleAnimation: function () {
        ...
        if (this.get(STATE) === IDLE) {
          var enemy =
            this.display.getEntity("Enemy") || this.display.getEntity("NPC");
          enemy.say(
      ...
      consumeCommand: function () {
        if (this.commandsStack && this.commandsStack.length > 0) {
          ...
          switch (command.type) {
            case "updated":
              Y.mix(this.findObject(command.object.id), command.object, true); // Update target object cfg
              this.consumeCommand();
              break;
            case "gameWon":
      ...
      highlight: function (loc, error) {
        ...
        if (typeof loc === "number") {
          ...
          session.addGutterDecoration(loc, "proggame-currentgutterline");
      ...
      addEditorTab: function (label, code, file) {
        ...
        aceField.on(
          ...
          Y.bind(function (e) {
            ...
            if (
              e.domEvent.target.className.indexOf("ace_gutter-cell") === -1 ||
            ...
            function (e) {
              ...
              if (tab) {
                ...
                Wegas.Facade.Variable.sendRequest({
                  ...
                  request:
                    "/Inbox/Message/" + e.file.get(ID) + "?view=Extended",
                  cfg: {
                    updateCache: false,
                  },
                  on: {
      ...
      showMessage: function (level, message) {
        ...
        Y.later(50, this, function () {
          ...
          Y.one("body").once(
            ...
            function () {
              panel.destroy();
              if (String(this.get("root").get("@pageId")) === "10") {
                this.showTutorial();
              } else {
                this.focusCode();
              }
          ...
          items: {
            ...
            properties: {
              ...
              id: {
                ...
                view: {
                  label: "Id",
                  description: "MUST be unique accross current map",
                },
              },
              x: {
                type: NUMBER,
