...
class Matrix extends React.Component<IMatrixProps> {
    ...
    toggle(x: number, y: number) {
        const {
            value,
            onChange,
            view: { valueToBool = defaultValueToBool, boolToValue = defaultBoolToValue },
        } = this.props;
    ...
    resize = () => {
        ...
        const newValue = tmpValue.map((ey, yI) => {
            return ey.map((ex, xI) => {
                return yI < s[1] && xI < s[0] ? value[yI][xI] : boolToValue(false);
            });
        });
        onChange(newValue);
    }
                        ...
                        <div key={yi} {...rowStyle}>
                            {row.map((col, xi) => (
                                <div
                                    ...
                                    style={{
                                        backgroundColor: valueToBool(col) ? trueColor : falseColor,
                                    }}
                                />
                            ))}
                        </div>
