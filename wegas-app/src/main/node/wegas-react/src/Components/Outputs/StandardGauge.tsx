...
export function StandardGauge({
  ...
  // Change to defaultColors if no colors are given
  const sectionsColor = colors?.length ? colors : defaultSectionsColor;
  const maxValue = Math.max(...sectionsColor.map(c => c.stopValue));
