...
export class WegasClient {
  ...
  oldInstantiate<T extends IMergeable | undefined | null>(
    entity: T,
  ): T extends IMergeable ? ScriptableEntity<T> : undefined {
    if (entity) {
      if (entity['@class'] in mapAtClassToConcreteClasses) {
        const atClass = entity['@class'] as keyof AtClassToConcreteClasses;
        return new mapAtClassToConcreteClasses[atClass](
          this,
          entity as any,
        ) as any;
        ...
        throw Error(
          'Cannot instantiate abstract class ' + entity['@class'] + '!',
        );
      }
    }
  ...
  oldInstantiateArray<T extends IMergeable[] | null | undefined>(
    entities: T,
  ): T extends (infer U)[]
  ...
  oldInstantiateMap<T extends MapOf<IMergeable> | null | undefined>(
    entities: T,
  ...
  test() {
    const listDesc: IListDescriptor = {
      ...
      editorTag: '',
      comments: '',
      defaultInstance: { '@class': 'ListInstance', version: 1 },
      version: 1,
      label: { '@class': 'TranslatableContent', version: 1, translations: {} },
    };
