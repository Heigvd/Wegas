...
YUI.add('alba-pageeditor', function(Y) {
    ...
    Y.extend(AlbaEditorPlugin, Y.Plugin.Base, {
        ...
        _initOverlays: function() {
            ...
            this._highlightOverlay = new Alignable({
                zIndex: 1, 
                render: true, 
                visible: false
            });								// Init the highlighting overlay
            this._highlightOverlay.get(BOUNDINGBOX).setStyles( {
                position: 'relative', 
                display:"none"
            });
            this._highlightOverlay.get(CONTENTBOX).setStyles( {
                position: 'absolute', 
                width:'100%', 
                opacity: '0.5', 
                height:'20px',
            ...
            AlbaEditorPlugin._contextMenu = new ContextMenu({
                zIndex: 2, 
                render: true, 
                visible: true
            });					// Init the context menu widget instance
            ...
            AlbaEditorPlugin._contextMenu.menu.addItem({
                text: ".", 
                submenu:{
                    itemdata:[{
                        text:"."
    ...
    _showOverlay: function(targetWidget) {
        ...
        this._highlightOverlay.get(CONTENTBOX).setStyles({
            height: targetNode.getComputedStyle('height')
        });
