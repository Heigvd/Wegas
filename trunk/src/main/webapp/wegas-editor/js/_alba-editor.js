...
YUI.add('alba-editor', function(Y) {
	...
	syncPagesTab: function() {
		...
		if (ret.children.length == 0) {
		    ret.children.push({
			label: "<em>This element does not have children</em>"
		    });
		}
		return ret;
	    }
	...
	initTableTab: function(element, columnDefs, dataSource, editionFormFields) {
	    var dataTable = new YAHOO.widget.DataTable(element._node, columnDefs, dataSource, {
		dynamicData: true
	    }); 
	...
	initTextsTab: function(element) {
		...
		key:"text", 
		sortable:true, 
		resizeable:true /*, editor: new YAHOO.widget.TextboxCellEditor({disableBtns:true})*/
	    }
	    ], dataTable,
	    textArray = [];
	...
	_addWire:function(srcStateCfg, transitionCfg) {
		...
		src: {
		    moduleId: srcStateIndex, 
		    terminal: "out"
		}, 
		tgt: {
		    moduleId: tgtModuleIndex, 
		    terminal: "in"
		}, 
		xtype: xtype, 
		label: label
	...
	syncStateMachineTab: function() {
		...
		cState.container = this.wireitlayer.addContainer({
			...
			},											// straight, arrows, bezier, bezierArrows
			offsetPosition: [-15, 10], 
			direction: [-0.5, 1]				
	...
	renderUI: function () {
	    ...
	    parentWidgetNode.one('.yui3-tabview-panel').setStyles( {
		height: panelHeight+'px'
	    });				// Set the panel node withe the right height
		...
		this.get('subpanelNode').setStyles({
		    height: subpanelHeight+"px"
		});								// Set the subpanel node withe the right height
	    } else {
